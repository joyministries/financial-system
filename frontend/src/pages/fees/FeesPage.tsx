  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-4">
        <select value={selectedGrade} onChange={(e) => setSelectedGrade(e.target.value)} className="rounded-lg border border-gray-300 px-3 py-2 text-sm">
      ...
      <Modal open={showForm} onClose={closeForm} title="New Fee Structure">
        <form onSubmit={handleCreate} className="space-y-4">
          <div>
            ...
            <select value={category} onChange={(e) => setCategory(e.target.value)} className="mt-1 block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm">
              {CATEGORIES.map((c) => <option key={c} value={c}>{c}</option>)}
            </select>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700">Annual Amount (R)</label>
            <input type="number" step="0.01" min="0.01" value={annual} onChange={(e) => setAnnual(e.target.value)} required className="mt-1 block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700">Payment Plan</label>
            <div className="mt-1 flex gap-2">
              <button type="button" onClick={() => setPaymentPlan('monthly')} className={`rounded-lg px-3 py-2 text-sm font-medium ${paymentPlan === 'monthly' ? 'bg-primary-600 text-white' : 'border border-gray-300 text-gray-700 hover:bg-gray-50'}`}>
                Monthly installments
              </button>
              <button type="button" onClick={() => setPaymentPlan('yearly')} className={`rounded-lg px-3 py-2 text-sm font-medium ${paymentPlan === 'yearly' ? 'bg-primary-600 text-white' : 'border border-gray-300 text-gray-700 hover:bg-gray-50'}`}>
                Pay yearly (lump sum)
              </button>
            </div>
          </div>
          {paymentPlan === 'monthly' ? (
            <div>
              <label className="block text-sm font-medium text-gray-700">Monthly Installment (R)</label>
              <input type="number" step="0.01" min="0.01" value={monthly} onChange={(e) => setMonthly(e.target.value)} required className="mt-1 block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm" />
            </div>
          ) : (
      ...
      <div className="rounded-xl bg-white shadow-sm border border-gray-100 overflow-hidden">
        {loading ? (
          ...
          <>
              ...
              <tbody className="divide-y divide-gray-200">
                {fees.map((f) => (
                  <tr key={f.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4 text-sm font-medium text-gray-900">{f.category}</td>
                    <td className="px-6 py-4">
                      <span className={`inline-flex rounded-full px-2 py-1 text-xs font-medium ${f.payment_plan === 'yearly' ? 'bg-blue-100 text-blue-700' : 'bg-purple-100 text-purple-700'}`}>
                        {f.payment_plan === 'yearly' ? 'Yearly' : 'Monthly'}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-700">R {Number(f.annual_amount).toLocaleString()}</td>
                    <td className="px-6 py-4 text-sm text-gray-700">{f.monthly_installment ? `R ${Number(f.monthly_installment).toLocaleString()}` : <span className="text-gray-400">Lump sum</span>}</td>
            ...
            {fees.length === 0 && <p className="py-8 text-center text-sm text-gray-500">{selectedGrade ? 'No fees for this grade/year.' : 'Select a grade to view fees.'}</p>}
          </>
        )}
      </div>
    </div>
